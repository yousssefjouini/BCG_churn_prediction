...
class Preprocessor:
    ...
    def clean_dates(self):
        ...
        self.df['date_invoice'] = pd.to_datetime(self.df['date_invoice'], format='%Y-%m-%d')
        self.df['year']=self.df['date_order'].dt.year
        self.df['month']=self.df['date_order'].dt.month
        self.df['difference_invoice_order']=(self.df["date_invoice"]-self.df["date_order"]).dt.days
    ...
    def add_rfm(self) -> pd.DataFrame:
        ...
        rfm.loc[(rfm['f_score']>3) & (rfm['r_score']>3) & (rfm['m_score']>3),'cluster']='champions'
        rfm.loc[(rfm['f_score']<2.5) & (rfm['r_score']<2.5) & (rfm['m_score']<2.5),'cluster']='to be lost'
        rfm.loc[(rfm['f_score']<2) & (rfm['r_score']>4) & (rfm['m_score']<2),'cluster']='new comers'
        return rfm

    def price_most_bought_product(self):
        most_bought_product_by_client = self.df.groupby('client_id',as_index=False).apply(lambda x: x.loc[x['quantity'].idxmax()])
        most_bought_product_by_client = most_bought_product_by_client[['client_id','sales_net']]
        most_bought_product_by_client = most_bought_product_by_client.rename(columns={'sales_net': 'price_most_bought_product'})
        return most_bought_product_by_client

    def total_sales_tot_by_client_branch(self): #compute total sales per client and branch
        total_sales_tot_by_client_and_branch = self.df.groupby(['client_id', 'order_channel'],as_index=True)[['sales_tot']].sum()
        total_sales_tot_by_client_and_branch = total_sales_tot_by_client_and_branch.unstack(level=-1)
        new_cols = ['_'.join(map(str, col)) for col in total_sales_tot_by_client_and_branch.columns]
        total_sales_tot_by_client_and_branch.columns = new_cols
        total_sales_tot_by_client_and_branch=total_sales_tot_by_client_and_branch.fillna(0).reset_index()
    ...
    def n_unique_products_client(self):
        ...
        number_of_products_bought_by_client = self.df.groupby('client_id',as_index=False)[['product_id']].nunique()
        number_of_products_bought_by_client.rename(columns={'product_id': 'n_unique_products_bought'},inplace=True)
    ...
    def identify_churn(self):
        ...
        cutoff_date = max(max_order_date_by_client['date_order']) - pd.Timedelta(days=180)
        max_order_date_by_client['churn']=0
        max_order_date_by_client.loc[max_order_date_by_client.date_order <= cutoff_date,'churn']=1
    ...
    def full_preprocessing(self):
        ...
        output = pd.merge(most_bought_product_by_client, total_sales_tot_by_client_branch, on='client_id')
    ...
    def draw_clusters(self):
        ...
        melted_rfm = pd.melt(self.rfm.reset_index(),id_vars=['client_id','cluster'],value_vars=['r_score','f_score','m_score'],var_name= 'Features',value_name= 'Value')
