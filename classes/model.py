...
from sklearn.metrics import classification_report, roc_auc_score, roc_curve, confusion_matrix, f1_score, plot_roc_curve
...
class ChurnClassifier:
    ...
    def _prepare_data(self):
        ...
        self.X_train,self.y_train  = self.train_data.drop(['churn', 'client_id','recency','r_score','cluster'], axis=1),self.train_data['churn']
        self.X_test, self.y_test   = self.test_data.drop(['churn', 'client_id','recency','r_score','cluster'], axis=1),self.test_data['churn']
    ...
    def auc(self):
        print("AUC score:", roc_auc_score(self.y_test, self.model.predict_proba(self.X_test)[:, 1]))
        fpr, tpr, thresholds = roc_curve(self.y_test, self.model.predict_proba(self.X_test)[:, 1])
        plt.plot(fpr, tpr)
    ...
    def output(self,path):
        output = self.test_data[['client_id','cluster','r_score','m_score','f_score']]
