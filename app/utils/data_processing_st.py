...
def preprocess_data(rfm_output_FILEPATH: str, transactions_aggregated_FILEPATH: str):
    """
    Performs preprocessing on the predictions (rfr_output) and on the transactions. Then, joins both tables.
    """
    # load and merge data
    rfm_output = pd.read_csv(rfm_output_FILEPATH)
    transactions_aggregated = pd.read_csv(transactions_aggregated_FILEPATH)
...
def make_violin_plots(data, client_id, fig_width=10, fig_height=6):
    ...
    ax = sns.violinplot(data=data[["r_score", "m_score", "f_score"]],
    ...
    ax.axhline(y=this_client[0], xmin=0.0659, xmax=0.2659, color="red", label="Client", linewidth=2)
    ...
    ax.set_title(f"RFM scores for this client vs all clients")
